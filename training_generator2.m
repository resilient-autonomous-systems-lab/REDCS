function gen_net = training_generator2(i_epoch, gen_net,dis_weights,alpha,thresholds,loss_curve_param)
...
f1_out = f1(dis_weights,out,thresh_1);
f2_out = f2(dis_weights,out,thresh_2);
...
for ind = 1:mini_batch_size:n_samples
    ...
    [gradients,net_state,loss] = dlfeval(@model_loss,gen_net,dis_weights,Z_iter, beta*mini_batch_size,thresh_1, thresh_2); % forward propogation, simulation, loss calculation, gradient calculation
...
function [gradients,states,loss] = model_loss(net,dis_weights,Z,beta_n,thresh_1,thresh_2)

[g_theta, states] = forward(net,Z);
loss    = relu((sum(exp(f1(dis_weights,g_theta,thresh_1))) - beta_n)) + ...
           relu((sum(exp(f2(dis_weights,g_theta,thresh_2))) - beta_n));

gradients = dlgradient(loss,net.Learnables);

function out = f1(weights,x,thresh_1)
y = sum(weights.*kernel_fcn(x),1);
out =  y - thresh_1;  % inidicator function outer circle


function out = f2(weights,x,thresh_2)
y = sum(weights.*kernel_fcn(x),1);
out =  thresh_2 - sum(y,1);  % inidicator function inner ci
