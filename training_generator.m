function gen_net = training_generator(i_epoch, gen_net,stealth_net,effect_net,alpha,thresholds,sim_param,loss_curve_param)
...
e = zeros(size(H,1),n_samples);
e(attack_indices,:) = out;
e = dlarray(e,"CB");
f1_out = f1(stealth_net,e,thresh_1);
f2_out = f2(effect_net, e,thresh_2);
pre_score = sum((f1_out<=0) & (f2_out<=0))/n_samples;
disp("pre-trained score = " + num2str(pre_score) + " ::: Target = " + num2str(alpha))

perf_fig = figure;
y_stealth = predict(stealth_net,e);
y_effect  = predict(effect_net,e);
subplot(121)
...
for ind = 1:mini_batch_size:n_samples
    ...
    [gradients,net_state,loss] = dlfeval(@model_loss,gen_net,Z_iter, beta*mini_batch_size,stealth_net,effect_net,thresh_1, thresh_2,sim_param); % forward propogation, simulation, loss calculation, gradient calculation
...
figure(perf_fig),
y_stealth = forward(stealth_net,e);
y_effect  = forward(effect_net,e);
subplot(121)
...
function [gradients,states,loss] = model_loss(net,Z,beta_n,stealth_net,effect_net,thresh_1,thresh_2,sim_param)
...
e = zeros(size(H,1),size(g_theta,2));
e(attack_indices,:) = double(g_theta);
e = dlarray(e,"CB");

%% calculate loss
loss    = relu((sum(exp(f1(stealth_net,e,thresh_1))) - beta_n)) + ...
           relu((sum(exp(f2(effect_net,e,thresh_2))) - beta_n));
